...
contract Night is Ownable {
  ...
  function collectTolls() external {
    require(msg.sender == tollCollector, "Night: Not toll collector");
  ...
  function sendEth(
    ...
    require(_tollCommitment == toll, "Night: Invalid or outdated toll commitment");

    // also protects from underflow
    require(msg.value > toll, "Night: Must pay more than the toll");
  ...
  function sendToken(
    ...
    require(msg.value == toll, "Night: Must pay the exact toll");
    require(tokenPayments[_receiver][_tokenAddr] == 0, "Night: Cannot send more tokens to stealth address");
  ...
  function withdrawToken(address _acceptor, address _tokenAddr) external {
    _withdrawTokenInternal(msg.sender, _acceptor, _tokenAddr, address(0), 0, INightHookReceiver(0), "");
  }
  ...
  function withdrawTokenOnBehalf(
    ...
    _validateWithdrawSignature(
      ...
      _sponsor,
      _sponsorFee,
      INightHookReceiver(0),
      "",
      _v,
      _r,
      _s
    );
    _withdrawTokenInternal(_stealthAddr, _acceptor, _tokenAddr, _sponsor, _sponsorFee, INightHookReceiver(0), "");
  }
  ...
  function _withdrawTokenInternal(
    ...
    // also protects from underflow
    require(_amount > _sponsorFee, "Night: No balance to withdraw or fee exceeds balance");
  ...
  function _validateWithdrawSignature(
    ...
    require(_recoveredAddress != address(0) && _recoveredAddress == _stealthAddr, "Night: Invalid Signature");
  }
}
